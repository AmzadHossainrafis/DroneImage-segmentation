...
class TrainPipeline:
    ...
    def __init__(self) -> None:
        ...
        self.model = smp.Unet(
            "resnet34", encoder_weights="imagenet", classes=22, activation=None
        ).to(config["Train_config"]["device"])
    ...
    def runner(self):
        ...
        train_dataset = self.DroneDataset(
            config["Dir"]["original_data_path"],
            config["Dir"]["label_path"],
            train,
            train_augmentation,
        )
        valid_dataset = self.DroneDataset(
            config["Dir"]["original_data_path"],
            config["Dir"]["label_path"],
            val,
            val_augmentatinon,
        )
        test_dataset = self.DroneDataset(
            config["Dir"]["original_data_path"],
            config["Dir"]["label_path"],
            test,
            test_augmentation,
        ...
        model_evaluation = ModelEvaluation(
            config["Dir"]["model_dir"], config["Train_config"]["device"], loss
        )
